    ...
    try:
        ...
        if code != 0:
            ...
            if result == '。' or result == '.。' or result == ' .。' or result == ' 。':
                ...
                if "下一页" in result or "下页" in result:
                    nextPage()
                    print("结果: %s,检测到关键词，翻下一页." % (result))
                elif "上一页" in result or "上页" in result:
                    lastPage()
                    print("结果: %s,检测到关键词，翻上一页." % (result))
                else:
                    print("结果: %s" % (result))
...
def run():
    global wsParam
    wsParam = Ws_Param(APPID='7b3bd222', APIKey='8373e5b8c755c5f15651da63947aebfa',
                       APISecret='NmMwZGFlMTc0YjFiZWUwYTQzNDY5ZGQ0')
